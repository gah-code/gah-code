# Gilberto A Haro

**Web Engineer & Technologist**  
Designing and engineering systems where content, interfaces, and automation meet.

I focus on building **scalable frontend architectures**, **content-driven platforms**, and **AI-assisted workflows** that improve how teams ship, manage, and evolve digital experiences.

---

## Focus Areas

- **Frontend Design Systems**  
  Component architecture, styling systems, accessibility, performance

- **Content Management Operations**  
  Headless CMS, content modeling, governance, automation

- **Marketing Technology**  
  SEO-aware builds, analytics-friendly systems, conversion-driven UI

- **AI-Driven Development**  
  Agent-assisted workflows, tooling automation, and applied LLM systems

---

## Toolkit

> Tools are instruments, not identities — these are the ones I reach for most often.

[![My Skills](https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,gatsby,graphql,nodejs,vite,sass,styledcomponents,tailwind,redux,jest,python,postgres,mongodb,supabase,cloudflare,git,bash,postman,md,obsidian,ps&perline=7)](https://skillicons.dev)

---

## How I Work

- Systems over snippets  
- Documentation as a first-class artifact  
- Performance, accessibility, and clarity as defaults  
- Continuous learning through real projects and experiments  

Many of my repositories reflect **in-progress thinking**, experiments, or reusable internal tooling—not just polished demos.

---

## Connect

<a href="https://github.com/gah-code" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-24292e?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://www.linkedin.com/in/gilbert-haro-2b108222b/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-1E77B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<a href="https://codepen.io/Gilbert-Haro" target="_blank">
  <img src="https://img.shields.io/badge/CodePen-131417?style=for-the-badge&logo=codepen&logoColor=white" />
</a>
<a href="https://www.figma.com/design/wQj36CdFHUEHxVd82tcbZU/Wireframing-in-Figma?node-id=107-275&t=Y0nCSIuXbcpE6ECT-1" target="_blank">
  <img src="https://img.shields.io/badge/Figma-000000?style=for-the-badge&logo=figma&logoColor=white" />
</a>
<a href="https://www.instagram.com/g_optics/?hl=en" target="_blank">
  <img src="https://img.shields.io/badge/Instagram-000000?style=for-the-badge&logo=instagram&logoColor=white" />
</a>

---

## GitHub Activity

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=gah-code&show_icons=true&theme=transparent&hide_title=true" height="150" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=gah-code&layout=compact&theme=transparent" height="150" />

</div>


